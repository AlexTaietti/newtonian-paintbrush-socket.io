...
//canvas
const trailCanvas = document.getElementsByClassName("c-gravity__trails")[0]; // '[0]' is needed since getElementsByClassName returns an HTML collection
const particlesCanvas = document.getElementsByClassName("c-gravity__particles")[0];
...
function animate() {
	...
	star.update();
	if(settings.BOUNDS){
		if(star.x > cW - star.radius){ star.x = cW - star.radius; star.v.x = 0; }
		if(star.x < 0 + star.radius){ star.x = 0 + star.radius; star.v.x = 0; }
		if(star.y > cH - star.radius){ star.y = cH - star.radius; star.v.y = 0; }
		if(star.y < 0 + star.radius){ star.y = 0 + star.radius; star.v.y = 0; }
	}
